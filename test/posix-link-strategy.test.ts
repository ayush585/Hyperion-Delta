...
import {
  PosixLinkStrategy,
  type PosixLinkStrategyAdapter,
} from "../src/internal/posix-link-strategy.js";
...
function createDelegatingAdapter(overrides: Partial<PosixLinkStrategyAdapter> = {}) {
  const adapter: PosixLinkStrategyAdapter = {
    linkSync,
    copyFileSync,
    renameSync,
    rmSync,
...
describe("PosixLinkStrategy", () => {
  it("backs up regular files through hard links and detaches the workspace target", () => {
    const workspaceRoot = createTempRoot("hyperion-posix-link-workspace-");
    const storageRoot = createTempRoot("hyperion-posix-link-storage-");
