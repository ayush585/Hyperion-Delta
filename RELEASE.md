...
5. Review `npm pack --dry-run`.
6. Confirm the tarball only includes `dist`, `README.md`, `ARCHITECTURE.md`, `LIMITATIONS.md`, `CHANGELOG.md`, the README benchmark hero image, and npm metadata.
7. Confirm the package install smoke imports both `HyperionWorkspace` and `HyperionAgentSession` from the packed tarball.

## Reliability SLO gate

- Silent corruption incidents: `0`
...
3. Create a GitHub release for the version, or manually dispatch the `Publish Package` workflow from `main` with `tag=refs/tags/vX.Y.Z`.
4. Let CI run `npm run release:final`.
5. The workflow publishes with `npm publish --provenance`.
